...
function type(d){
    ...
    return{
        ...
        geners:JSON.parse(d.genres).map(d=>d.name),
...
function setupCanvas(BarChartData,movieClean){
 ...
 function update(data){
    ...
    .rangeRound([0,chart_height]).padding(0.25);
...
// const yScale=d3.scaleBand().domain(BarChartData.map(d=>d.genre)).rangeRound([0,chart_height]).paddingInner(0.25);
    let yScale=d3.scaleBand().domain(BarChartData.map(d=>d.title)).rangeRound([0,chart_height]).paddingInner(0.15);
...
// header.append('tspan').text('Total revenue by genre in $us');
    ...
    let xAxis=d3.axisTop(xScale_v3).ticks(5).tickFormat(formatTicks).tickSizeInner(-chart_height).tickSizeOuter(0);
    ...
    function mouseover(e){
        ...
        const bodyData=[
            ...
            ['TMBD Popularity',Math.round(thisBarData.popularity)],
            ['TMDB Rating',thisBarData.vote_average],
            ['Genres', thisBarData.geners.join(', ')]
        ];
